...
#
# joblib.dump(train, 'pre2_df_train.pkl', compress=3)
# joblib.dump(test, 'pre2_df_test.pkl', compress=3)
#
# max_features = 120000
...
#
# # embedding_matrixはbilstmのものを使い回す
#
# ########
#
# # embedding_matrix = joblib.load('embedding_matrix.pkl')
...
# joblib.dump(train_preds, 'valid_pred_pytorch.pkl', compress=3)
...
te_preds0.append(joblib.load("test_pred_bilstm.pkl")[:,0][:, np.newaxis])
te_preds0.append(joblib.load("test_pred_pytorch.pkl")[:,0][:, np.newaxis])
te_preds1.append(joblib.load("test_pred_bilstm.pkl")[:,1][:, np.newaxis])
te_preds1.append(joblib.load("test_pred_pytorch.pkl")[:,1][:, np.newaxis])
te_preds2.append(joblib.load("test_pred_bilstm.pkl")[:,2][:, np.newaxis])
te_preds2.append(joblib.load("test_pred_pytorch.pkl")[:,2][:, np.newaxis])
te_preds3.append(joblib.load("test_pred_bilstm.pkl")[:,3][:, np.newaxis])
te_preds3.append(joblib.load("test_pred_pytorch.pkl")[:,3][:, np.newaxis])
print(te_preds0[0].shape)
print(te_preds0[1].shape)
va_preds = np.hstack(va_preds)
te_preds0 = np.hstack(te_preds0)
te_preds1 = np.hstack(te_preds1)
te_preds2 = np.hstack(te_preds2)
te_preds3 = np.hstack(te_preds3)
y_va = joblib.load("df_train.pkl").target.values[:, np.newaxis]
print(va_preds.shape)
# print(te_preds.shape)
va_preds_merged, te_preds_merged0 = merge_predictions(X_tr=va_preds, y_tr=y_va, X_te=te_preds0)
...
delta, f1_score = bestThresshold(y_va, va_preds_merged)
print('[Model mean] best threshold is {:.4f} with F1 score: {:.4f}'.format(delta, f1_score))
va_preds_merged, te_preds_merged2 = merge_predictions(X_tr=va_preds, y_tr=y_va, X_te=te_preds2)
delta, f1_score = bestThresshold(y_va, va_preds_merged)
print('[Model mean] best threshold is {:.4f} with F1 score: {:.4f}'.format(delta, f1_score))
va_preds_merged, te_preds_merged3 = merge_predictions(X_tr=va_preds, y_tr=y_va, X_te=te_preds3)
delta, f1_score = bestThresshold(y_va, va_preds_merged)
print('[Model mean] best threshold is {:.4f} with F1 score: {:.4f}'.format(delta, f1_score))

te_preds_merged = (te_preds_merged0 + te_preds_merged1 + te_preds_merged2 + te_preds_merged3)/ 4
